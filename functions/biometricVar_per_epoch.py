...
#   ~ example: {'1': [[0, 99], dataframe]}, If the pre-determined biometric variable was 'HR', then the dataframe would have as index 0-99 and as columns rolling averaged HR spanning the index(0-99) in the input dataframe (eeg_data).  
...
def biometricVar_per_epoch(epoch_dict, eeg_data):
    ...
    epochbm_dict={}
    # list to store column names we want to access from the data frame

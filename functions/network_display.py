...
def nodesEdges(full_list):
    ...
    # edge_cons = []
    # for elec in full:
    #     edge_cons.extend(list(itertools.combinations(elec,2)))

    # node_class = []
    # for i,j in enumerate(edge_cons):
    #     node_class.append(j[0]+j[1])

    # list_edge_cons = [list(x) for x in edge_cons]
    # for i,j in enumerate(list_edge_cons):
    #     # print(i,j)
    #     list_edge_cons[i].append(node_class[i])

    # # building nodes
    # unique_elecs = list(set(list(itertools.chain(*full))))
    ...
    #     }
    #     for short, label in (
    #         list(zip(unique_elecs,unique_elecs))
    #     )
    # ]
